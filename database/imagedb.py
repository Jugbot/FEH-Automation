...
def featureTemplateMatch(image, template):
    ...
    src_pts = np.float32([kp1[m.queryIdx].pt + (1,) for m in matches]).reshape(-1, 3)
    dst_pts = np.float32([kp2[m.trainIdx].pt + (1,) for m in matches]).reshape(-1, 3)
...
def strToImage(txt, font_file=FONT_FILE, font_size=128):

    font = ImageFont.truetype(font_file, font_size)
    font_size = font.getsize(txt)

    img = Image.new('L', (font_size[0], font_size[1]), (128,))
    draw = ImageDraw.Draw(img)
    # font = ImageFont.truetype(font_file, font_size)
    draw.text((0,0), txt, (0,), font)
    return np.array(img)
